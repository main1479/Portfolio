...
process.stdin.on('end', () => {
  ...
  if (/\.env(\.local|\.production|\.development|\.preview)?$/.test(filePath) && !/\.example/.test(filePath)) {
    violations.push('CRITICAL: Attempting to write a .env file via code. Manage .env files manually.');
  }

  // CRITICAL: API keys, secrets, tokens assigned to variables
  if (/(?:api[_-]?key|api[_-]?secret|secret[_-]?key|access[_-]?token|auth[_-]?token|private[_-]?key)\s*[:=]\s*['"][A-Za-z0-9_\-./+]{16,}['"]/i.test(content)) {
  ...
  if (/['"](?:re_|sk_live_|sk_test_|pk_live_|ghp_|github_pat_|xoxb-|xoxp-)[A-Za-z0-9_\-]{16,}['"]/.test(content)) {
    violations.push('CRITICAL: Hardcoded service key pattern detected (Resend / Stripe / GitHub / Slack). Use process.env.*.');
  }
  ...
  if (/dangerouslySetInnerHTML/.test(content) && !/sanitize|DOMPurify|xss/i.test(content)) {
    violations.push('HIGH: dangerouslySetInnerHTML without visible sanitization. Use DOMPurify or similar, and leave a comment.');
  }
  ...
  if (/localStorage\s*\.\s*setItem\s*\(\s*['"](?:token|auth|session|jwt|access)/i.test(content)) {
    violations.push('HIGH: Storing auth-shaped tokens in localStorage is insecure. Prefer httpOnly cookies.');
  }

  if (violations.length > 0) {
    console.error('[Security Hook] VIOLATIONS DETECTED:\n' + violations.map(v => '  - ' + v).join('\n'));
