...
A focused SCSS-only tweak. The site's biggest display-tier headings (`.display-xl`, `.page-intro__title`, the End-CTA heading, and the Footer "Mainul" wordmark) cap at 24–27rem (240–270px) once viewport width hits ~1500px. On a normal 1080p / 1440p desktop this looks confident; on a 2560px+ ultrawide it just looks oversized — the type sits in the middle of a much wider canvas and reads as shouting.
...
| `app/_components/Footer/_Footer.module.scss:123`     | Footer big mark      | `clamp(9rem, 22vw, 24rem)`   | `clamp(9rem, 22vw, 20rem)`   |
...
- Footer big mark → `14rem` (slightly smaller — it sits next to the giant signature wordmark, doesn't need to dominate)

---

## Scope and non-scope

- **In scope:** the four selectors above. These are the only display-tier rules with a max ≥20rem.
...
- One implementation commit (mechanical edits across 4 files).
