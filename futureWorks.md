...
- [motion] Case-study pages get only the template-level page fade-up on entrance. A per-line yPercent reveal on `CaseHero` titles (matching the home Hero + PageIntro language) was explicitly deferred in the spec to avoid restructuring multi-line `<h1>`s with mixed accent/outline styling. Revisit if cases feel under-animated next to the rest of the site. — `feature/page-transitions` session 2026-05-26
- [visual] Page-transitions feature ships with full typecheck + lint + build green, but the sliding panel sweep, page fade-up, PageIntro title reveal, and reduced-motion variants were not visually verified by Claude this session (no browser-driving tool available). Human-eyeball pass needed on every route — `/`, `/about`, `/contact`, `/work`, `/work/avsb`, `/cv` (verify static), `/404`, plus back/forward nav, hash-only nav, and a reduced-motion-on pass. — `feature/page-transitions` session 2026-05-26
